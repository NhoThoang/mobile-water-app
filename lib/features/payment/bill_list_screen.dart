...
class _BillListScreenState extends State<BillListScreen> {
  ...
  Future<void> _fetchBills() async {
    ...
    try {
      final response = await apiClient.dio.get("/payments/history"); // Endpoint lấy lịch sử bill
      if (response.statusCode == 200) {
        setState(() {
          _bills = (response.data as List).map((j) => Bill.fromJson(j)).toList();
        });
      }
    } catch (e) {}
  ...
  void _showPaymentQR(Bill bill) {
    ...
    String qrUrl = "https://api.qrserver.com/v1/create-qr-code/?size=250x250&data=Thanh+toan+BILL+${bill.id}";
        ...
        child: Column(
          ...
          children: [
            ...
            Container(
              ...
              child: Text("BILL ${bill.id}", style: const TextStyle(fontWeight: FontWeight.bold)),
            ),
            const SizedBox(height: 30),
            const Text("Hệ thống sẽ tự động cập nhật sau 1-2 phút chuyển khoản.", 
                  ...
                  child: Column(
                    children: [
                      Row(
                        ...
                        children: [
                          Text("Tháng ${bill.month}", style: const TextStyle(fontWeight: FontWeight.bold, fontSize: 18)),
                          Container(
                      ...
                      Row(
                        ...
                        children: [
                          const Text("Tiêu thụ:"),
                          Text("${bill.consumption} m3", style: const TextStyle(fontWeight: FontWeight.bold)),
                        ],
                      ),
                      const SizedBox(height: 10),
                      Row(
